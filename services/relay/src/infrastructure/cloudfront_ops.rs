...
/// CloudFrontディストリビューション有効化結果
#[derive(Debug, Clone)]
pub struct EnableDistributionResult {
    /// ディストリビューションID
    pub distribution_id: String,
    /// 成功したかどうか
    pub success: bool,
...
impl EnableDistributionResult {
    /// 成功結果を作成
    pub fn success(distribution_id: impl Into<String>, was_disabled: bool) -> Self {
        let id = distribution_id.into();
        let message = if was_disabled {
            format!("ディストリビューション {} を有効化しました", id)
        } else {
            format!("ディストリビューション {} は既に有効化されています", id)
        };
        Self {
            distribution_id: id,
            success: true,
    ...
    pub fn failure(distribution_id: impl Into<String>, error: impl std::fmt::Display) -> Self {
        let id = distribution_id.into();
        let message = format!("ディストリビューション {} の有効化に失敗: {}", id, error);
        Self {
            distribution_id: id,
            success: false,
            was_disabled: false,
...
pub trait CloudFrontOps: Send + Sync {
    ...
    /// CloudFrontディストリビューションを有効化する
    ///
    /// GetDistributionでETagと現在の設定を取得し、
    /// UpdateDistributionでEnabled=trueに設定する。
    ///
    /// # 引数
    /// * `distribution_id` - CloudFrontディストリビューションID
    ///
    /// # 戻り値
    /// * `Ok(EnableDistributionResult)` - 有効化結果
    /// * `Err(CloudFrontOpsError)` - エラー
    async fn enable_distribution(
        &self,
        distribution_id: &str,
    ) -> Result<EnableDistributionResult, CloudFrontOpsError>;
}
...
impl CloudFrontOps for AwsCloudFrontOps {
    ...
    async fn enable_distribution(
        ...
        // 現在の状態を確認
        let was_disabled = !distribution_config.enabled();

        // 既に有効化されている場合はスキップ
        if !was_disabled {
            info!(
                distribution_id = %distribution_id,
                "ディストリビューションは既に有効化されています"
            );
            return Ok(EnableDistributionResult::success(distribution_id, false));
        }
        ...
        match update_result {
            Ok(_) => {
                info!(
                    distribution_id = %distribution_id,
                    "UpdateDistribution成功（有効化）"
                );
                Ok(EnableDistributionResult::success(distribution_id, true))
            }
            Err(err) => {
                warn!(
                    distribution_id = %distribution_id,
...
mod tests {
    ...
    impl MockCloudFrontOps {
        ...
        fn with_distribution(
            ...
            self.distributions
                .insert(distribution_id.into(), (enabled, operation_success));
            self
        }
    ...
    impl CloudFrontOps for MockCloudFrontOps {
        ...
        async fn enable_distribution(
            ...
            match self.distributions.get(distribution_id) {
                ...
                Some((enabled, true)) => {
                    Ok(EnableDistributionResult::success(distribution_id, !*enabled))
                }
                Some((_, false)) => Err(CloudFrontOpsError::AwsSdkError("mock error".to_string())),
                None => Err(CloudFrontOpsError::DistributionNotFound(
                    distribution_id.to_string(),
    ...
    // ==================== EnableDistributionResult テスト ====================

    #[test]
    fn test_enable_distribution_result_success_was_disabled() {
        let result = EnableDistributionResult::success("E1234567890ABC", true);

        assert_eq!(result.distribution_id, "E1234567890ABC");
        assert!(result.success);
        assert!(result.was_disabled);
        assert!(result.message.contains("E1234567890ABC"));
        assert!(result.message.contains("有効化しました"));
    }

    #[test]
    fn test_enable_distribution_result_success_was_enabled() {
        let result = EnableDistributionResult::success("E1234567890ABC", false);

        assert_eq!(result.distribution_id, "E1234567890ABC");
        assert!(result.success);
        assert!(!result.was_disabled);
        assert!(result.message.contains("既に有効化"));
    }

    #[test]
    fn test_enable_distribution_result_failure() {
        let result = EnableDistributionResult::failure("E1234567890ABC", "API error");
    ...
    // ==================== enable_distribution テスト ====================

    #[tokio::test]
    async fn test_mock_cloudfront_ops_enable_success_was_disabled() {
        // enabled=false -> was_disabled=true
        let mock = MockCloudFrontOps::new().with_distribution("E1234567890ABC", false, true);

        let result = mock.enable_distribution("E1234567890ABC").await;
    ...
    #[tokio::test]
    async fn test_mock_cloudfront_ops_enable_success_was_enabled() {
        // enabled=true -> was_disabled=false
        let mock = MockCloudFrontOps::new().with_distribution("E1234567890ABC", true, true);
    ...
    #[tokio::test]
    async fn test_mock_cloudfront_ops_enable_failure() {
        let mock = MockCloudFrontOps::new().with_distribution("E1234567890ABC", false, false);
    ...
    #[tokio::test]
    async fn test_mock_cloudfront_ops_enable_not_found() {
        let mock = MockCloudFrontOps::new();

        let result = mock.enable_distribution("E-nonexistent").await;
